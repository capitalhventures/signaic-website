...
const starterQueries = [
  { category: "Regulatory", icon: Scale, query: "What are the latest FCC spectrum allocation decisions affecting satellite operators?" },
  { category: "Competitive", icon: Satellite, query: "Compare SpaceX and Amazon Kuiper's constellation strategies" },
  { category: "Financial", icon: DollarSign, query: "Summarize recent SEC filings from major defense contractors" },
  { category: "Orbital", icon: Globe2, query: "What new satellite deployments have been authorized this month?" },
  { category: "News", icon: Newspaper, query: "What are the biggest space and defense stories this week?" },
  { category: "Custom", icon: Sparkles, query: "Analyze the competitive landscape for on-orbit satellite servicing" },
];

export default function AskRaptorPage() {
  return (
    <Suspense fallback={<div className="flex items-center justify-center h-[calc(100vh-64px)]"><span className="text-slate-400">Loading...</span></div>}>
      <AskRaptorContent />
    </Suspense>
  );
}
...
function AskRaptorContent() {
  ...
  const [conversations] = useState<Conversation[]>([
    { id: "1", title: "NSSL Phase 3 Competition Analysis", updatedAt: "2 hours ago" },
    { id: "2", title: "SpaceX Starlink FCC Filing Review", updatedAt: "Yesterday" },
    { id: "3", title: "Defense Budget Impact Assessment", updatedAt: "3 days ago" },
  ]);
                    ...
                    >
                      {msg.content || (
                        <span className="inline-flex items-center gap-1 text-slate-400">
                          <span className="w-1.5 h-1.5 bg-brand-cyan rounded-full animate-pulse-glow" />
                          Analyzing...
                        </span>
                      )}
                    </div>
          ...
          <div className="max-w-3xl mx-auto">
            ...
            {messages.length > 0 && (
              <div className="flex items-center gap-2 mt-3">
                ...
                <button
                  onClick={() => handleSend("What are the strategic implications?")}
