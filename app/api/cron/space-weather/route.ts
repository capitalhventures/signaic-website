...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!res.ok) {
      await logCronExecution("space_weather", "error", 0, "NOAA API returned " + res.status);
      return cronError("NOAA API returned " + res.status);
    }

    const alerts: NOAAAlert[] = await res.json();
    if (!Array.isArray(alerts) || alerts.length === 0) {
      await logCronExecution("space_weather", "success", 0);
      return cronSuccess({ source: "space_weather", inserted: 0 });
    }
    ...
    await logCronExecution("space_weather", "success", totalInserted);
    return cronSuccess({ source: "space_weather", inserted: totalInserted });
  } catch (err) {
    const message = err instanceof Error ? err.message : "Unknown error";
    await logCronExecution("space_weather", "error", 0, message);
    return cronError("Failed to refresh space weather data");
  }
}
